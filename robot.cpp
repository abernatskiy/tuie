...
double ROBOT::Get_Compass_Sensor_Value(void) {
	...
	/*
        ...
        double vect1Length = sqrt( pow(vect1[0],2.0) + pow(vect1[1],2.0) );
        double vect2Length = sqrt( pow(vect2[0],2.0) + pow(vect2[1],2.0) );

        double dotProduct = vect1[0]*vect2[0] + vect1[1]*vect2[1];

        double cosTheta = dotProduct / (vect1Length*vect2Length);
...
void ROBOT::Neural_Network_Set_Sensors(void) {
	...
	// Throw away first proprioceptive sensor value and replace with the compass sensor value.

	neuralNetwork->Sensor_Set(sensorIndex,Get_Compass_Sensor_Value());
	sensorIndex++;
