...
//		for (int s=0; s<numSensors; s++) {
		for (int s=0; s<11; s++) {
		// Don't use the position of the robot's main body.
...
*/
	...
	for (int i=0;	i<numMotors;	i++) {
		...
		neuronValues->vals[i] = temp->vals[i];

		neuronValues->vals[i] = atan(neuronValues->vals[i]) / (3.14159/2.0);
                // Scale neuron values to lie in [-1,1]
	}

//	if ( timeStep==0 )
//		sensorValues->Print(10);
}
